...
/**
 * Tiered cache for report payloads: Redis → Memcached → miss (caller queries DB).
 */
class Report_cache_store {

	/** @var CI_Controller */
	protected $ci;

	/** @var string */
	protected $key_prefix = 'attend_report_';

	/** @var string|null */
	protected $last_read_source;
	...
	public function __construct($config = [])
	{
		...
		$this->ci->config->load('report_cache', true, true);
		$cfg = $this->ci->config->item('report_cache');
		if (is_array($cfg) && !empty($cfg['key_prefix'])) {
	...
	{
		$this->last_read_source = null;
		$key = $this->_key($key);

		$data = $this->_read_redis($key);
		if (is_array($data)) {
			$this->last_read_source = 'redis';
			return $data;
		}

		$data = $this->_read_memcached($key);
		if (is_array($data)) {
			$this->last_read_source = 'memcached';
			return $data;
		}
	...
	/**
	 * @return string|null redis|memcached
	 */
	public function lastReadSource()
	{
		return $this->last_read_source;
	}
