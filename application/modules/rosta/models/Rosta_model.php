...
public function __Construct(){
	...
	Public function fetch_report($date_range,$start=NULL,$limit=NULL,$employee=NULL){	
		...
		if($employee!==''){
			$empidf="and ihrisdata.ihris_pid='$employee'";
		}
		else
		{
			$empidf="";
		...
		if($department!==''){
			$depr_filter="and dutyreport.department_id='$department'";
		}
		else
		{
			$depr_filter="";
		...
		if(!empty($department)){
		$all=$this->db->query("select distinct dutyreport.ihris_pid from dutyreport,ihrisdata where dutyreport.facility_id='$facility' and ihrisdata.ihris_pid=dutyreport.ihris_pid and dutyreport.duty_date like '$valid_range-%' $depr_filter $div_filter $funit $search LIMIT $limit,$start");
		}
		else{
			$all=$this->db->query("select distinct dutyreport.ihris_pid from dutyreport,ihrisdata where dutyreport.facility_id='$facility' and ihrisdata.ihris_pid=dutyreport.ihris_pid and dutyreport.duty_date like '$valid_range-%'  $depr_filter $div_filter $funit  $search LIMIT $limit,$start");
		}
		...
		foreach($rows as $row){
			...
			if(!empty($department)){  //if department has been defined
	...
	if(!empty($this->department)){
		$all=$this->db->query("select distinct ihris_pid from dutyreport where dutyreport.facility_id='$facility'and dutyreport.department_id='$department' and dutyreport.duty_date like '$valid_range-%'");
		}
		else{
	...
	Public function fetch_tabs($date_range,$start,$limit,$employee=FALSE){	
		...
		if($rowno<1){
			...
			if(!empty($department)){
			$all=$this->db->query("select distinct ihrisdata.ihris_pid from ihrisdata,dutyreport where ihrisdata.facility_id='$facility' $dep_filter $div_filter $funit $search LIMIT $limit,$start"); //apply limits
			}
			else{
				$all=$this->db->query("select distinct ihrisdata.ihris_pid from ihrisdata,dutyreport where ihrisdata.facility_id='$facility' $dep_filter $div_filter $funit $search LIMIT $limit,$start");
			}
	...
	Public function getActuals(){
		...
		//and schedules.purpose='a'
		if($department){
	    $query=$this->db->query("select actuals.*,schedules.letter as actual from actuals,schedules where(actuals.schedule_id=schedules.schedule_id) and actuals.ihris_pid ");
