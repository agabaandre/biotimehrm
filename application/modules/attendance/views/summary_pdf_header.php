...
<head>
	...
	<style>
		...
		table.summary-table { width: 100%; border-collapse: collapse; font-size: 9pt; margin-top: 6px; }
		table.summary-table th, table.summary-table td { border: 0.5px solid #7f8c8d; padding: 6px 8px; vertical-align: middle; }
		table.summary-table thead th { background: #1a5276; color: #fff; font-weight: bold; text-align: center; font-size: 8.5pt; }
		table.summary-table thead th.text-left { text-align: left; }
		table.summary-table tbody td { background: #fff; }
		table.summary-table tbody tr:nth-child(even) td { background: #f4f6f7; }
		table.summary-table .num { text-align: center; }
		table.summary-table .name-col { text-align: left; }
	</style>
</head>
<body>
	<table class="report-header" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td>
				...
				<p class="report-subtitle"><?php echo isset($_SESSION['facility_name']) ? htmlspecialchars($_SESSION['facility_name']) : ''; ?> — <?php echo isset($period_label) ? htmlspecialchars($period_label) : (isset($dates) ? date('F, Y', strtotime($dates . '-01')) : date('F, Y')); ?></p>
			</td>
		</tr>
	</table>
