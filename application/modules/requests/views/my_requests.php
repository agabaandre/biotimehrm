          ...
          <div class="card-body">
            <form id="filterForm" class="row g-3">
              ...
              <div class="col-md-4">
                ...
                <input type="text" class="form-control" id="searchInput" placeholder="Search by name, reason, facility...">
              </div>
                  ...
                  <?php if (!empty($requests)): ?>
                    <?php foreach ($requests as $request): ?>
                      <tr>
                        ...
                        <td class="text-center"><?php echo str_replace("person|", "", $request->ihris_pid); ?></td>
                        ...
                        <td class="text-center">
                          <?php if ($request->status == "Pending"): ?>
                            <?php if ($user['ihris_pid'] !== $request->ihris_pid): ?>
                              <div class="btn-group" role="group">
                                <a href="<?php echo base_url(); ?>requests/acceptRequest/<?php echo str_replace('|', '_', $request->entry_id); ?>" 
                                   class="btn btn-success btn-sm" title="Accept Request">
                                  <i class="fas fa-check"></i>
                                </a>
                                <a href="<?php echo base_url(); ?>requests/rejectRequest/<?php echo str_replace('|', '_', $request->entry_id); ?>" 
                            ...
                            <?php endif; ?>
                          <?php else: ?>
                            <span class="text-muted"><?php echo $request->status; ?></span>
                          <?php endif; ?>
                        </td>
                      </tr>
                    <?php endforeach; ?>
...
$(document).ready(function() {
    ...
    $('#resetFilters').on('click', function() {
        $('#statusFilter, #dateFilter').val('').trigger('change');
        $('#searchInput').val('');
        table.search('').draw();
    });
