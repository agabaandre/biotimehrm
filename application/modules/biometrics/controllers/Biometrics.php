...
class Biometrics extends MX_Controller{
    ...
    public function getEnrolledAjax(){
        // Clear any previous output
        if (ob_get_level()) {
            ob_end_clean();
        }
        ...
        try {
            ...
            $draw = $this->input->post('draw') ? intval($this->input->post('draw')) : 1;
            $start = $this->input->post('start') ? intval($this->input->post('start')) : 0;
            $length = $this->input->post('length') ? intval($this->input->post('length')) : 20;
            ...
            // Get filtered count (with search) for recordsFiltered
            $recordsFiltered = $this->biometrics_mdl->getEnrolledCount($search);
            ...
            if (!empty($users) && is_array($users)) {
                foreach($users as $user) {
                    $ihris_pid = isset($user->ihris_pid) ? str_replace('person|', '', $user->ihris_pid) : '';
                    $fullname = isset($user->fullname) ? $user->fullname : '';
                    $othername = isset($user->othername) ? $user->othername : '';
                    $name = trim($fullname . ' ' . $othername);
                    $facility = isset($user->facility) ? $user->facility : '';
                    $device = isset($user->device) ? $user->device : '';
                    $job = isset($user->job) ? $user->job : '';
                    $card_number = isset($user->card_number) ? $user->card_number : '';
                    $att_status = isset($user->att_status) ? intval($user->att_status) : 0;
                    $status_html = $att_status == 1 ? "<p style='color:green;'>Active</p>" : "<p style='color:red;'>In-Active</p>";
            ...
            ob_end_clean();
            log_message('error', 'getEnrolledAjax Exception: ' . $e->getMessage());
            log_message('error', 'Stack trace: ' . $e->getTraceAsString());
            ...
            log_message('error', 'getEnrolledAjax Fatal Error: ' . $e->getMessage());
    ...
    public function getUnenrolledAjax(){
        // Clear any previous output
        if (ob_get_level()) {
            ob_end_clean();
        }
        ...
        try {
            ...
            // Get total count (without search) for recordsTotal
            $recordsTotal = $this->biometrics_mdl->getUnenrolledCount('');
            
            // Get filtered count (with search) for recordsFiltered
            $recordsFiltered = $this->biometrics_mdl->getUnenrolledCount($search);
            
            // Get paginated data
            $users = $this->biometrics_mdl->getUnenrolledPaginated($start, $length, $search, $order);
            ...
            ob_end_clean();
            log_message('error', 'getUnenrolledAjax Exception: ' . $e->getMessage());
            log_message('error', 'Stack trace: ' . $e->getTraceAsString());
            ...
            log_message('error', 'getUnenrolledAjax Fatal Error: ' . $e->getMessage());
